<template>
  <section class="bg-primary-yellow-light py-40">
    <div
      class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1  lg:grid-cols-2 grid-x-10"
    >
      <div class="bg-white px-20 lg:px-40 py-20 rounded-xl">
        <nuxt-link to="/contact-us" class="app-button bg-primary-yellow text-white">
          Join the journey
        </nuxt-link>
      </div>
      <div class=" flex items-center justify-center">
        <h4 class="text-primary-green text-left font-bold font-roboto text-3xl">
          Building an Africa to last <br/>generations
        </h4>
      </div>
    </div>
  </section>
</template>

<script>
export default {};
</script>

<style scoped></style>
